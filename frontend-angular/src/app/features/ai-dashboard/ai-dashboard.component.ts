          ...
          @if (ocrResult()!.success) {
            <div class="prescription-sheet" style="margin-top:24px; background:#fff; border:1px solid #cfd8dc; border-radius:8px; box-shadow:0 8px 24px rgba(0,0,0,.08); padding:30px; font-family:'Courier New', monospace; position:relative; overflow:hidden;">
              ...
              <div style="text-align:center; margin-bottom:24px; border-bottom:2px solid #0077B6; padding-bottom:12px;">
                ...
                <p style="margin:4px 0 0; font-size:11px; text-transform:uppercase; color:#546e7a;">Sihati - Numérisation Clinique</p>
              </div>
              ...
              <div style="display:grid; grid-template-columns:1fr 1fr; gap:20px; margin-bottom:30px; font-size:13px; line-height:1.6; color:#37474f;">
                <div>
                  <strong style="color:#0077B6;">Médecin Prescripteur :</strong><br>
                  <span style="font-size:14px; font-weight:bold;">{{ ocrResult()!.doctorName || 'Dr. Non Détecté' }}</span>
                </div>
                <div style="text-align:right;">
                  <strong style="color:#0077B6;">Date d'Émission :</strong><br>
                  <span style="font-size:14px; font-weight:bold;">{{ ocrResult()!.prescriptionDate || '--/--/----' }}</span>
                </div>
              </div>

              <!-- Medications List -->
              <div style="margin-bottom:30px;">
                <h4 style="margin:0 0 10px; font-size:14px; color:#0077B6; border-bottom:1px dashed #cfd8dc; padding-bottom:6px;">TRAITEMENTS PRESCRITS</h4>
...
export class AiDashboardComponent implements OnInit, AfterViewChecked, OnDestroy {
  ...
  importMedications() {
    ...
    const meds = ocr.detectedMedications || [];
    const dosages = ocr.detectedDosages || [];
    ...
    const requests = meds.map((medName, index) => {
      const dosage = dosages[index] || 'Selon avis médical';
      const body = {
        name: medName,
        dosage: dosage,
        frequency: 'Voir ordonnance',
        instructions: `Importé automatiquement depuis ordonnance OCR - ${doctorName}`
      };
    ...
    forkJoin(requests).subscribe({
      next: () => {
        this.snack.open(`${meds.length} médicament(s) importé(s) avec succès !`, 'OK', {duration: 3000, panelClass: 'snack-ok'});
        this.resetOcr();
      },
      error: (err) => {
